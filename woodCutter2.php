	...
	******************************************************************************/
	...
	while($maxCut->parentid != 0){
		...
		if($result ->num_rows > 0 ) {
			while($row = $result->fetch_assoc()) {
    			...
    			echo "<h1>" .$inventoryObject->mycount . " here</h1>";
    			echo "<h1>Plus " . $quan . "</h1>";
    			$quan+=$inventoryObject->mycount;
    			echo "<h1>New Total " . $quan . "</h1>";
	...
	echo "<tr><th>Quantity</th><th>Height</th><th>Width</th><th>Length</th><th>Value</th></tr>";
		...
		foreach($FinalSummary2 as $key=>$cutval){
			echo "<tr><td>" . $key  . "</td><td>" .$cutval->height . "</td><td>" . $cutval->width. "</td><td>". $cutval->length."</td><td>$" 

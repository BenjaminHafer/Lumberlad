	...
	******************************************************************************/
				...
				foreach($area as $x => $x_val){
					...
					foreach($lumberTypes as $lum1){ // foreach 1
						...
						if($cut2->val > 0){
					  		...
					  		$key = $cut2->id;
				  			array_push($temp,$cut2->areas);
					  		$bestcuts[$key] = $cut2;
					   		$idnum++;
					 }//close lumber for each
	...
	echo "<tr><th>Quantity</th><th>Height</th><th>Width</th><th>Length</th><th>Value</th></tr>";
		...
		foreach($FinalSummary2 as $key=>$cutval){
			echo "<tr><td>" . number_format($cutval->quan,2)  . "</td><td>" .number_format($cutval->height,2) . "</td><td>" 
			. number_format($cutval->width,2). "</td><td>". number_format($cutval->length,2)."</td><td>$" 
